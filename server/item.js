...
const itemStore = new ItemStore({ filename: 'src/db/items.json', autoload: true });
export const itemRouter = new Router();

itemRouter.get('/', async (ctx) => {
  const userId = ctx.state.user._id;
  console.log(userId);
  let items = await itemStore.find({ userId });
...
const createItem = async (ctx, item, response) => {
  try {
    ...
    item.userId = userId;
    item._id = `${uuid()}`;
    let created_item = await itemStore.insert(item);
    response.body = created_item;
    response.status = 201; // created
    created_item._id = response.body._id;
